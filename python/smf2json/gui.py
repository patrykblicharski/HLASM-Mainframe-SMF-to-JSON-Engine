...
class SmfApp(tk.Tk):
    ...
    def _build_menu(self) -> None:
        ...
        file_m.add_command(label="Open SMF dump…", command=self.open_dump)
        file_m.add_separator()
        file_m.add_command(label="Export JSON…", command=self.export_json)
    ...
    def _build_body(self) -> None:
        ...
        self.debug = tk.Text(dbg_frame, height=10, wrap=tk.WORD, font=("Consolas", 9))
    ...
    def _on_tree_motion(self, event: tk.Event) -> None:  # type: ignore[type-arg]
        ...
        if region not in ("heading", "cell") or not col_id:
            self.tooltip.hide()
            return
        try:
        ...
        if idx < 0 or idx >= len(self.columns):
            self.tooltip.hide()
            return
        key = self.columns[idx]
        desc = self.descriptions.get(key, "")
        tip = f"{key}\n{desc}" if desc else key
        self.tooltip.show(tip, event.x_root + 12, event.y_root + 12)

    def export_json(self) -> None:
        if not self.rows:
            messagebox.showinfo("Export", "No data to export — open a dump first.")
            return
